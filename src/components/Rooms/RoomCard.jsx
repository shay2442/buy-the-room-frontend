...
const RoomCard = ({ room, addToCart, handleDelete }) => {
    ...
    <div>
      <Card>
        <div className="card">
          ...
          <div><b>City:</b> {room.city}</div>
          <div><b>State:</b> {room.state}</div>
          <div><b>Description:</b> {room.description}</div>
          <div><b>Price :$ </b>{room.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
          ...
          <button onClick={ () => navigate(`/rooms/${room.id}/edit`)}>Edit</button>
...
const Card = styled.div`
  ...
  button {color: rgb(119, 134, 148); font-size: 25px; line-height: 15px; padding: 8px; border-radius: 10px; font-family: Georgia, serif; font-weight: normal; text-decoration: none; font-style: normal; font-variant: normal; text-transform: none; background-color: white; display: inline-block; margin-bottom: 0px;
